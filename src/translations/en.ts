export default {
  // General
  replacement_voucher: 'Replacement Voucher',
  select_return_items: 'Select Return Items',
  enter_new_items: 'Enter New Items',
  returned_items: 'Returned Items',
  new_items: 'New Items',
  value: 'Value',
  total_returned_value: 'Total Returned Value',
  total_new_value: 'Total New Value',
  financial_difference: 'Financial Difference',
  amount_due_to_supplier: 'Amount due to supplier',
  amount_due_from_supplier: 'Amount due from supplier',
  no_financial_difference: 'No financial difference',
  please_add_returned_items: 'Please add returned items first',
  replacement_voucher_processed: 'Replacement voucher processed successfully',
  error_processing_voucher: 'Error processing voucher',
  select_item_manually: 'Select Item Manually',
  select_item: 'Select an item',
  barcode_scanned: 'Barcode Scanned',
  barcode: 'Barcode',
  scan_error: 'Scan Error',
  review_and_confirm: 'Review and Confirm',
  back: 'Back',
  next: 'Next',
  finish: 'Finish',
  welcome: 'Welcome',
  dashboard: 'Dashboard',
  urgent_actions: 'Urgent Actions',
  items_expiring_soon: 'Items Expiring Soon',
  items_at_reorder_point: 'Items at Reorder Point',
  count: 'Count',
  add_invoice: 'Add Invoice',
  scan_single_item: 'Scan Single Item',
  scan_continuously: 'Scan Continuously',
  no_items_added: 'No items added yet',
  please_select_store_first: 'Please select a store first',
  please_fill_header_fields: 'Please fill in the invoice header fields first',
  please_complete_all_fields_for_item: 'Please complete all fields for item',
  invoice_processed_successfully: 'Invoice processed successfully',
  error_saving_invoice: 'Error saving invoice',
  add_new_inventory_invoice: 'Add New Inventory Invoice',
  invoice_details: 'Invoice Details',
  invoice_details_description: 'Select the manufacturer, supplier, and destination store for this invoice',
  invoice_items: 'Invoice Items',
  scan_or_enter_barcode: 'Scan or enter barcode',
  pick_date: 'Pick a date',
  add_another_item: 'Add Another Item',
  save_invoice: 'Save Invoice',
  logout_successful: 'Logged out successfully',
  logout_failed: 'Logout failed',
  login_successful: 'Login successful',
  login_failed: 'Login failed',
  welcome_back: 'Welcome Back',
  login_to_continue: 'Log in to your account to continue',
  email: 'Email',
  login: 'Login',
  error_fetching_data: 'Error fetching data',
  invalid_or_insufficient_quantity: 'Invalid or insufficient quantity',
  transfer_failed: 'Transfer failed',
  not_found: 'Not Found',
  please_complete_all_fields_correctly: 'Please complete all fields correctly',
  error_processing_consumption: 'Error processing consumption record',
  dashboard_overview: 'Overview of your supplies and alerts',
  supplies_overview: 'Manage and track your medical supplies',
  calendar_overview: 'View supplies expiration dates',
  alerts_overview: 'Monitor critical expiry alerts',
  reports_overview: 'Generate and export reports',
  management: 'Management',
  admin: 'Administration',
  credentials: 'Credentials',
  logout: 'Logout',
  search_supplies: 'Search supplies...',
  coming_soon: 'Coming soon...',
  more: 'more',
  name: 'Name',
  description: 'Description',

  // Sidebar Headers
  inventory_management: 'Inventory',
  operations: 'Operations',
  analysis: 'Analysis',
  settings: 'Settings',
  monitoring_and_planning: 'Monitoring & Planning',

  // Navigation
  replacement_voucher_nav: 'Replacement Voucher',
  on_shelf_invoicing_nav: 'On-Shelf Invoicing',
  dashboard_nav: 'Dashboard',
  data_settings_nav: 'Data Settings',
  management_settings_nav: 'System Management',
  supplies_nav: 'Supplies',
  bulk_add_inventory_nav: 'Bulk Add Inventory',
  add_supply_nav: 'Add Supply',
  calendar_nav: 'Calendar',
  alerts_nav: 'Alerts',
  reports_nav: 'Reports',
  manufacturers_nav: 'Manufacturers',
  suppliers_nav: 'Suppliers',
  users_nav: 'Users',
  admin_settings_nav: 'Admin Settings',
  supply_types_nav: 'Supply Types',
  product_definitions_nav: 'Product Definitions',
  stores_nav: 'Stores',
  transfer_inventory_nav: 'Transfer Inventory',
  import_export_nav: 'Import/Export',
  procedure_templates_nav: 'Procedure Templates',
  supplier_performance_nav: 'Supplier Performance',
  returns_management_nav: 'Returns Management',

  // Transfers
  transfer_inventory: 'Transfer Inventory',
  transfer_details: 'Transfer Details',
  select_source_destination: 'Select source and destination stores to begin',
  from_store: 'From Store',
  to_store: 'To Store',
  scanning: 'Scanning...',
  start_scanning: 'Start Scanning',
  stop_scanning: 'Stop Scanning',
  items_to_transfer: 'Items to Transfer',
  confirm_transfer: 'Confirm Transfer',
  confirm_item_quantity: 'Confirm Item Quantity',
  available_quantity: 'Available Quantity',
  quantity_to_transfer: 'Quantity to Transfer',
  add_to_list: 'Add to List',
  select_stores_first: 'Please select source and destination stores first',
  stores_must_be_different: 'Source and destination stores must be different',
  item_not_in_source_store: 'This item was not found in the source store',
  transfer_successful: 'Transfer successful',

  // Inventory Report
  inventory_report_nav: 'Inventory Report',
  stock_balance: 'Stock Balance',
  all_stores: 'All Stores',
  group_by: 'Group By',
  group_by_product: 'Product',
  group_by_manufacturer: 'Manufacturer',

  // Reorder Point Report
  reorder_point_report_nav: 'Reorder Point Report',
  reorder_point_report: 'Reorder Point Report',
  items_below_reorder_point: 'Items Below Reorder Point',
  current_stock: 'Current Stock',

  // Consumption Report
  consumption_report_nav: 'Consumption Report',
  on_shelf_report_nav: 'On-Shelf Report',
  consumption_rate_report: 'Consumption Rate Report',
  consumption_analysis: 'Consumption Analysis',
  group_by_department: 'Department',
  total_consumed_quantity: 'Total Consumed Quantity',

  // Stores
  stores: 'Stores',
  add_store: 'Add Store',
  edit_store: 'Edit Store',
  store_name: 'Store Name',
  location: 'Location',
  confirm_delete_store_desc: 'Are you sure you want to delete this store?',

  // Dashboard
  total_supplies: 'Total Supplies',
  expiring_soon: 'Expiring Soon',
  expired: 'Expired',
  valid_supplies: 'Valid Supplies',
  by_type: 'By Type',
  recent_activities: 'Recent Activities',
  supplies_expiring_soon: 'Supplies Expiring Soon',
  latest_supplies: 'Latest Supplies',

  // Supply Status
  expired_status: 'Expired',
  expiring_soon_status: 'Expiring Soon',
  valid: 'Valid',
  needs_replacement_action_status: 'Replacement Action Needed',

  // Calendar
  expiry_calendar: 'Expiry Calendar',
  today: 'Today',
  selected_date: 'Selected Date',
  no_date_selected: 'No Date Selected',
  click_on_date: 'Click on a date to view details',
  no_expiries: 'No Expiries',
  no_expiries_on_date: 'No supplies expiring on this date',
  critical: 'Critical',
  warning: 'Warning',

  // Alerts
  replacement_action_alerts: 'Replacement Alerts',
  critical_alerts: 'Critical Alerts',
  warning_alerts: 'Warning Alerts',
  expired_items: 'Expired Items',
  days_remaining: 'days remaining',
  no_critical_alerts: 'No Critical Alerts',
  no_warning_alerts: 'No Warning Alerts',
  no_expired_items: 'No Expired Items',
  all_supplies_safe: 'All supplies are safe from expiration',
  all_supplies_valid: 'All supplies are valid',

  // Days of the week
  saturday: 'Sat',
  sunday: 'Sun',
  monday: 'Mon',
  tuesday: 'Tue',
  wednesday: 'Wed',
  thursday: 'Thu',
  friday: 'Fri',

  // Notifications
  expiry_alert: 'Expiry Alert',
  supplies_expiring_alert: 'You have 8 supplies expiring in the next 30 days',

  // Actions
  add_new_supply: 'Add New Supply',
  view_all: 'View All',
  view_details: 'View Details',
  no_supplies: 'No supplies found',
  no_supplies_found: 'No supplies found',
  try_different_filters: 'Try different filters or clear search',
  clear_filters: 'Clear Filters',
  new_supplies_added: 'New supplies added',
  hours_ago: '{0} hours ago',

  // Filters and Sorting
  filter_by_status: 'Filter by Status',
  filter_by_type: 'Filter by Type',
  all_statuses: 'All Statuses',
  all_types: 'All Types',
  showing: 'Showing',
  of: 'of',
  supplies: 'supplies',
  sort_by: 'Sort by',
  earliest: 'Earliest',
  latest: 'Latest',
  lowest: 'Lowest',
  highest: 'Highest',

  // Supply Form
  add_inventory_item: 'Add New Inventory',
  add_new_inventory: 'Add New Inventory',
  inventory_form_description: 'Enter the details of the new inventory shipment',
  product: 'Product',
  select_product: 'Select a product',
  variant: 'Variant',
  select_variant: 'Select a variant',
  store: 'Store',
  select_store: 'Select a store',
  scan_barcode_to_find: 'Scan Barcode to Find',
  product_found: 'Product Found',
  enter_quantity: 'Enter Quantity',
  supply_form_description: 'Enter the details of the new medical supply',
  supply_type: 'Supply Type',
  supply_size: 'Size',
  serial_number: 'Serial Number',
  batch_number: 'Batch Number',
  batch: 'Batch',
  quantity: 'Quantity',
  expiry_date: 'Expiry Date',
  purchase_price: 'Purchase Price',
  manufacturer: 'Manufacturer',
  supplier: 'Supplier',
  alert_period: 'Alert Period',
  save: 'Save',
  cancel: 'Cancel',
  reset: 'Reset',
  select: 'Select',

  // Reports
  reports: 'Reports',
  generate_report: 'Generate Report',
  report_type: 'Report Type',
  inventory_report_desc: 'View and analyze current stock levels.',
  reorder_point_report_desc: 'Identify items that have reached their reorder point.',
  consumption_report_desc: 'Track and analyze material consumption rates.',
  expiry_report_desc: 'Monitor the expiration dates of supplies.',
  reports_hub_description: 'Your reporting hub. Choose a report to view.',
  view_report: 'View Report',
  date_range: 'Date Range',
  export_as_pdf: 'Export as PDF',
  export_as_csv: 'Export as CSV',
  expiry_report: 'Expiry Report',
  inventory_report: 'Inventory Report',
  supplier_report: 'Supplier Report',
  select_report_type: 'Select Report Type',
  select_report_options: 'Select options for your report',
  pick_a_date: 'Pick a date',
  charts: 'Charts',
  data: 'Data',
  supplies_by_status: 'Supplies by Status',
  supplies_by_type: 'Supplies by Type',
  report_data: 'Report Data',
  days: 'days',

  // Supply Types
  add_supply_type: 'Add Supply Type',
  edit_supply_type: 'Edit Supply Type',
  delete_supply_type: 'Delete Supply Type',
  type_name: 'Type Name',
  product_definitions: 'Product Definitions',
  add_definition: 'Add Definition',
  edit_definition: 'Edit Definition',
  product_name: 'Product Name',
  product_type: 'Product Type',
  select_type: 'Select a type',
  variant_label: 'Variant Label',
  variants: 'Variants',
  variant_name: 'Variant Name',
  reorder_point: 'Reorder Point',
  add: 'Add',
  variants_count: 'Variants Count',
  confirm_delete_product_def_desc: 'Are you sure you want to delete this product definition?',

  // Manufacturers
  add_manufacturer: 'Add Manufacturer',
  edit_manufacturer: 'Edit Manufacturer',
  delete_manufacturer: 'Delete Manufacturer',
  manufacturer_name: 'Name',
  manufacturer_alert_period: 'Default Alert Period (Days)',

  // Suppliers
  add_supplier: 'Add Supplier',
  edit_supplier: 'Edit Supplier',
  delete_supplier: 'Delete Supplier',
  supplier_name: 'Name',
  supplier_contact: 'Contact',
  supplier_phone: 'Phone',
  supplier_email: 'Email',
  supplier_alert_period: 'Replacement Alert Period (Days)',

  // Admin
  admin_settings: 'Admin Settings',
  username: 'Username',
  password: 'Password',
  change_credentials: 'Change Login Credentials',
  import_data: 'Import Data',
  export_data: 'Export Data',
  import: 'Import',
  export: 'Export',
  download_template: 'Download Template',
  upload_file: 'Upload File',

  // Confirmation
  confirm: 'Confirm',
  confirm_delete: 'Are you sure you want to delete this item?',
  confirm_delete_desc: 'This action cannot be undone.',
  yes_delete: 'Yes, Delete',

  // Alerts
  success: 'Success',
  error: 'Error',
  item_added: 'Item added successfully',
  item_updated: 'Item updated successfully',
  item_deleted: 'Item deleted successfully',
  error_processing_item: 'Error processing item',

  // Months
  month_1: 'Month',
  months_3: '3 Months',
  months_6: '6 Months',
  months_12: '12 Months',

  // Consumption
  consumption_records: 'Consumption Records',
  consumption_records_description: 'Track and manage supply consumption records',
  view_records: 'View Records',
  new_consumption: 'New Consumption',
  consumption_form_description: 'Add a new consumption record',
  consumption_nav: 'Consumption',
  medical_use: 'Medical Use',
  damaged_items: 'Damaged Items',
  purpose: 'Purpose',
  select_purpose: 'Select Purpose',
  requested_by: 'Requested By',
  enter_name: 'Enter Name',
  notes: 'Notes',
  enter_notes: 'Enter Notes',
  items: 'Items',
  add_item: 'Add Item',
  scan_barcode: 'Scan Barcode',
  supply: 'Supply',
  units: 'Units',
  no_consumption_records: 'No Consumption Records',
  create_first_consumption_record: 'Create your first consumption record',
  filter_by_department: 'Filter by Department',
  all_departments: 'All Departments',
  record_details: 'Record Details',
  cardiology_dept: 'Cardiology',
  surgery_dept: 'Surgery',
  emergency_dept: 'Emergency',
  radiology_dept: 'Radiology',
  icu_dept: 'ICU',
  pharmacy_dept: 'Pharmacy',
  record_deleted: 'Record deleted successfully',
  delete_failed: 'Failed to delete record',
  delete_consumption_confirm: 'Are you sure you want to delete this consumption record?',
  delete: 'Delete',
  consumption_record_created: 'Consumption record created successfully',
  insufficient_quantity: 'Insufficient quantity available',
  please_complete_all_fields: 'Please complete all required fields',
  date: 'Date',
  department: 'Department',
  actions: 'Actions',
  items_count: 'Items Count',

  // Inventory Report V2
  cost: "Cost",
  all_suppliers: "All Suppliers",
  filter_by_product_name: "Filter by product name...",
  filter_by_variant: "Filter by variant...",
  columns: "Columns",

  // Stock Types
  stock_type: "Stock Type",
  select_stock_type: "Select stock type",
  purchased: "Purchased",
  on_shelf: "On Shelf",
  voucher_number: "Voucher Number",
  on_shelf_invoicing: 'On-Shelf Invoicing',
  please_select_items_to_invoice: 'Please select items to invoice',
  invoice_created_successfully: 'Invoice created successfully',
  failed_to_create_invoice: 'Failed to create invoice',
  creating_invoice: 'Creating Invoice...',
  create_invoice: 'Create Invoice',
  consumption_date: 'Consumption Date',
  consumed_quantity: 'Consumed Quantity',
  unit_price: 'Unit Price',
  total_cost: 'Total Cost',
  total: 'Total',

  // On-Shelf Report
  on_shelf_items_status: 'On-Shelf Items Status',
  filter_by_supplier: 'Filter by Supplier',
  initial_quantity: 'Initial Quantity',
  remaining_quantity: 'Remaining Quantity',
  invoicing_status: 'Invoicing Status',
  not_consumed: 'Not Consumed',
  consumed_not_invoiced: 'Consumed, Not Invoiced',
  partially_invoiced: 'Partially Invoiced',
  fully_invoiced: 'Fully Invoiced',
  no_variants_added: 'No variants added yet',

  // Reorder Point Manager
  reorder_point_manager_nav: 'Reorder Point Manager',
  reorder_point_manager_desc: 'Manage minimum stock levels for all supplies',
  saved_successfully: 'Saved Successfully',
  update_failed: 'Update Failed',
  base_item: 'Base Item',
  class: 'Class',
  no_results_found: 'No results found',
  refresh: 'Refresh',
};
