...
- **المهام:**
    ...
    2.  **[x] تحسين دعم الباركود للأقواس والمسافات والشرطات:** تم توسيع قائمة تنسيقات الباركود المدعومة في `useBarcodeScanner.ts` لزيادة فرص الكشف عن الباركود الذي يحتوي على هذه الرموز.
    3.  **[x] معالجة خطأ عدم دعم Barcode Detector API:** تم تعديل `useBarcodeScanner.ts` و `AddSupplyPage.tsx` و `MobileSupplyItemCard.tsx` للتعامل بشكل رشيق مع حالة عدم دعم `Barcode Detector API` في المتصفح، وذلك بتعطيل وظيفة المسح وعرض رسالة توضيحية للمستخدم.
    4.  **[ ] بناء واجهة تقارير موحدة:** إعادة تصميم صفحة التقارير لتجميع كل التقارير (جرد، حد الطلب، استهلاك) في واجهة واحدة مع تبويبات.
    5.  **[ ] تطوير واجهة إضافة مخزون مجمّع:** إنشاء واجهة جديدة تسمح بإضافة عدة أصناف مخزون دفعة واحدة.
    6.  **[ ] ربط التقارير بالبيانات الحية:** التأكد من أن جميع التقارير تعمل بشكل ديناميكي مع Supabase.
